                ...
                function (resultado) {
                    ...
                    if (resultado.length == 1) {
                        bcrypt.compare(senha, usuario['senha'], function (err, result) {
                            if (result) {
                                let redirecionamento = usuario['fkAdmin'] == null ? 'dashboard/dashAnalista/index.html' : 'dashboard/dashManutencao/index.html';
...
function changePermission(req, res){
    const idUsuarioServer = req.body.idUsuarioServer;
    const fkAdminServer = req.body.idUsuarioServer;

    usuarioModel.changePermission(fkAdminServer, idUsuarioServer)
                .then(
                    function (resultado) {
                        res.json(resultado);
                    }
